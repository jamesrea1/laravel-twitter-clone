<div class="flex px-4 pt-3 pb-1">
    ...
    <div class="flex-auto min-w-0">
        <h5 class="flex items-center whitespace-nowrap">
            <a href="{{ $tweet->user->path() }}" class="group font-bold truncate inline-block" style="max-width: 80%">
                <span class="group-hover:underline">{{ $tweet->user->name }}</span>
                <span class="text-bluegray-500 font-normal ml-0.5">{{ ' @'.$tweet->user->username }}</span>
            </a>
            <span class="text-bluegray-500 font-normal block ml-1">· {{ $tweet->published_date }}</span>
        </h5>
        ...
        <div class="flex justify-between w-11/12 mt-1 -ml-2.5">
            ...
            <button type="submit" class="inline-flex items-center group cursor-pointer text-bluegray-500">
                <div class="w-9 h-9 flex items-center justify-center rounded-full transition-all 
                    group-hover:text-twitter group-hover:bg-twitter group-hover:bg-opacity-10
                ...
                <span class="text-xs ml-1 mr-2 transition-all group-hover:text-twitter" style="min-width:20px"></span>
            </button> 

            <button type="submit" class=" ">
                <div class="inline-flex items-center group cursor-pointer text-bluegray-500">
                    <div class="w-9 h-9 flex items-center justify-center rounded-full transition-all [ group-hover:text-green-600 group-hover:bg-green-600 group-hover:bg-opacity-10 ]">
                    ...
                    <span class="text-xs ml-1 mr-2 transition-all group-hover:text-green-600" style="min-width:20px"></span>
                </div>
            </button> 
            
            <form action="/tweets/{{ $tweet->id }}/likes" method="POST" style="" class="">
                @csrf
                @if ($tweet->isLikedBy(current_user()))
                    @method('DELETE')
                
                    <button type="submit" class=" ">
                        <div class="inline-flex items-center group cursor-pointer text-twrose">
                            <div class="w-9 h-9 flex items-center justify-center rounded-full transition-all [ group-hover:text-twrose group-hover:bg-twrose group-hover:bg-opacity-10 ]">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" ><g><path d="M12 21.638h-.014C9.403 21.59 1.95 14.856 1.95 8.478c0-3.064 2.525-5.754 5.403-5.754 2.29 0 3.83 1.58 4.646 2.73.814-1.148 2.354-2.73 4.645-2.73 2.88 0 5.404 2.69 5.404 5.755 0 6.376-7.454 13.11-10.037 13.157H12z"></path></g></svg>
                            </div>
                            <span class="text-xs ml-1 mr-2 transition-all group-hover:text-twrose" style="min-width:20px">{{ $tweet->likes }} </span>
                        </div>
                    </button> 
            ...
            <div class="flex">
                <a href="#" style="" class="w-9 h-9 flex items-center justify-center rounded-full [ text-bluegray-500 hover:text-twitter hover:bg-twitter hover:bg-opacity-10 ]">
                ...
                {{-- <span class="text-xs ml-1" style="min-width:20px"> </span> --}}
            </div>
